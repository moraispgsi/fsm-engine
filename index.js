...
module.exports = function (dialect, host, user, password, database) {
    ...
    let fsmCore = init(dialect, host, user, password, database);    //Initialize fsm-core
    ...
    meta.model.instance.belongsTo(fsmCore.model.version, {
    ...
    meta.model.snapshot.belongsTo(model.instance, {
    ...
    class Instance {
        ...
        startPromise() {
            return co(function*() {
                let instance = yield model.instance.findById(this.id);
                ...
                yield model.instance.update({hasStarted: true}, {where: {id: this.id}});
            }.bind(this));
        }
    }
    ...
    function _makeStateChart(versionID, snapshot) {
        return co(function*() {
            ...
            let isVersionSealed = yield fsmCore.action.isVersionSealed(versionID);
            if(!isVersionSealed){
                throw new Error("The Finite-state machine version is not sealed");
            }

            let version = yield fsmCore.model.version.findById(versionID); //Find the version with the versionID
    ...
    function _makeInstance(versionID, sc) {
        return co(function*() {
            ...
            return new Instance(sc, instanceID);
        })
    }
    ...
    return co(function*() {
        ...
        for (let instanceRow of instances) {
            ...
            yield fsmCore.assert.assertVersionSealed(versionID);            //Make sure the version is sealed
            ...
            let instance = new Instance(sc, instanceRow.dataValues.id); //Creates an instance object
